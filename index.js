	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `You, @${num.split('@')[0]} Suck my ass 🖕`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*MateuxinMaker Bot* : ${me.name}\n*17996562227* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Contato de bloqueio total* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}`
				...
				case 'nsfwloli':
					...
					break
				case 'hilih':
					if (args.length < 1) return reply('Cadê o texto, mulher?')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Cadê o url, perreco?')
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o url, vadia?')
